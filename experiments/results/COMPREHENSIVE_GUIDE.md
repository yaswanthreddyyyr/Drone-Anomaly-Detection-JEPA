...
```yaml
chunking:
  chunk_size: 1024      # Each chunk = 1024 waypoints
  stride: 512           # Overlap = 50% for smooth coverage
  min_chunk_size: 512   # Discard chunks < 512 waypoints
```

**Processing**:
```
Flight Log (2048 waypoints)
   ↓ [Chunking with stride=512]
   ├─ Chunk 0: waypoints [0:1024]
   ├─ Chunk 1: waypoints [512:1536]
...
encoder = MLP([
    1024 (input projection) →      # Coarse features
    768  (intermediate) →           # Medium features
    384  (fine embedding)           # High-level patterns
])  # 3 layers (vs 2 in v2)
...
Chunk sequence:
  ...
  [cls_emb, e1, e2, ..., e1024]  each 384-dim
...
│ 
├─ Shape: (1024 waypoints, 11 features)
...
│  └─ Output: (1024, 384) temporal embeddings
│
...
│
└─ Output: chunks of (1024, 11) shape, split labels
│
├──────────────────────────────────────────────────────────────────────
...
│  ├─ Input Chunk: (1024, 11)
│  │
│  ├─ Waypoint Embedding: (1024, 11) → (1024, 1024)
...
│  ├─ Input chunk: (1, 1024, 11)
│  ├─ Encoder: (1, 1024, 11) → (1, 1024, 384)
